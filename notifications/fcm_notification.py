...
from community.models import Post, PostComment, Qna, QnaAnswer, QnaAnswerComment
...
def send_fcm(uuid, title, body, data=None):
    ...
    if registration_token:
        if data:
            if data['type'] == '2':
                data['post_type'] = str(
                    Post.objects.get(id=data['link_id']).type)
            elif data['type'] == '3':
                data['qna_solved'] = str(
                    Qna.objects.get(id=data['link_id']).solved)
...
def exercise_attendance_fcm(uuid):
    ...
    if UserNotiReception.objects.get(user_id=uuid).exercise_attendance:
        data = {
            'type': '1',
        }
...
def daily_exercise_complete_fcm(uuid):
    ...
    if sum(1 for e in exercisePlans) > 0:
        if all(plan.completed for plan in exercisePlans):
            ...
            if UserNotiReception.objects.get(user_id=uuid).daily_exercise_complete:
                data = {
                    'type': '1',
                }
                send_fcm(uuid, title, body, data)
...
def weekly_exercise_protein_fcm(uuid):
    ...
    days = DailyExerciseStats.objects.filter(
        day__range=(start, end)).count()
    ...
    if days > 0:
        ...
        if UserNotiReception.objects.get(user_id=uuid).weekly_exercise_protein:
            data = {
                'type': '1',
            }
            send_fcm(uuid, title, body, data)
...
def hot_post_fcm(uuid, postId):
    ...
    if UserNotiReception.objects.get(user_id=uuid).hot_post:
        data = {
            'type': '2',
...
def hot_qna_fcm(uuid, qnaId):
    ...
    if UserNotiReception.objects.get(user_id=uuid).hot_qna:
        data = {
            'type': '3',
            'id': str(notification.id),
            'link_id': str(qnaId),
...
def qna_best_answer_fcm(userId, qnaId):
    ...
    if UserNotiReception.objects.get(user_id=userId).qna_best_answer:
        data = {
            'type': '3',
            'id': str(notification.id),
            'link_id': str(qnaId),
...
def qna_selected_answer_fcm(userId, qnaId):
    ...
    if UserNotiReception.objects.get(user_id=userId).qna_selected_answer:
        data = {
            'type': '3',
            'id': str(notification.id),
            'link_id': str(qnaId),
...
def activity_level_up_fcm(uuid, addedProtein):
    ...
    if staticData.activity_level < 6:
        if 0 <= (countData.count_protein - getLevelRequiredProtein(staticData.activity_level+1)) < addedProtein:
            ...
            if UserNotiReception.objects.get(user_id=uuid).activity_level_up:
                data = {
                    'type': '4',
                    'id': str(notification.id),
                }
...
def post_comment_fcm(uuid, postId, comment):
    ...
    if uuid != post.user_id:
        ...
        if UserNotiReception.objects.get(user_id=post.user_id).post_comment:
            data = {
                'type': '2',
                'id': str(notification.id),
                'link_id': str(postId),
...
def post_reply_fcm(uuid, postId, postCommentId, reply):
    ...
    if uuid != postComment.user_id:
        ...
        if UserNotiReception.objects.get(user_id=postComment.user_id).post_reply:
            data = {
                'type': '2',
...
def qna_answer_fcm(qnaId, answer):
    ...
    if UserNotiReception.objects.get(user_id=qna.user_id).qna_answer:
        data = {
            'type': '3',
            'id': str(notification.id),
            'link_id': str(qnaId),
...
def qna_comment_fcm(uuid, qnaId, qnaAnswerId, comment):
    ...
    if uuid != qnaAnswer.user_id:
        ...
        if UserNotiReception.objects.get(user_id=qnaAnswer.user_id).qna_comment:
            data = {
                'type': '3',
                'id': str(notification.id),
                'link_id': str(qnaId),
...
def qna_reply_fcm(uuid, qnaId, qnaAnswerId, qnaAnswerCommentId, reply):
    qnaAnswer = QnaAnswer.objects.get(id=qnaAnswerId)
    qnaAnswerComment = QnaAnswerComment.objects.get(id=qnaAnswerCommentId)

    title = '[Q&A]\n새로운 대댓글이 달렸어요'
    body = (reply[:20] + '..') if len(reply) > 20 else reply

    if uuid != qnaAnswerComment.user_id:
        qnaAnswerCommentNotification = Notification(
            user_id=qnaAnswerComment.user_id, title=title, body=body, type=3, link_id=qnaId)
        qnaAnswerCommentNotification.save()

        if UserNotiReception.objects.get(user_id=qnaAnswerComment.user_id).qna_reply:
            data = {
                'type': '3',
                'id': str(qnaAnswerCommentNotification.id),
                'link_id': str(qnaId),
            }

            send_fcm(qnaAnswerComment.user_id, title, body, data)
